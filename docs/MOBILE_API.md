...
| `POST` | `/api/v1/audience/presence` | Record consent-aware Web/iOS/Android/tvOS/Android TV/Roku installation presence |
...
Apple TV, Android TV and Roku do not collect a Clerk password. They create a ten-minute pairing request and display both a QR and a six-character sync code. A signed-in website or mobile app must show and explicitly confirm the identical code before the request can be exchanged once. Public television content does not require sign-in.
...
The mobile metrics response includes the same Web, iOS, Android, Apple TV, Android TV, Roku and developer-API audience summary shown in Studio.
...
The `/api/v1/stories` reader routes are reserved for same-origin requests from
`https://www.thejerseycourier.com`,
`https://api.thejerseycourier.com`, the permanent
`https://njc-web.vercel.app` alias, and identified official Courier clients.
The API hostname also provides the clean alias `/v1/stories`. Third-party and
server-to-server integrations must use `/api/developer/v1/stories` (or
`https://api.thejerseycourier.com/developer/v1/stories`) with an account API
key; reader-route headers are not a substitute for developer authentication.
