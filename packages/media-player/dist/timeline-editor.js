...
const defaultLabels = { eyebrow: "Playback composition", title: "Source timeline", description: "Mark the program itself. Inherited presentation media is offset automatically.", inheritedPresentation: "Inherited presentation", inheritedHelp: "Read-only here. The host application owns the active platform ident.", emptyPreview: "Attach a video source to unlock scrubbing and segment preview.", sourceTimeline: "Source timeline", zoom: "Zoom", overlapReview: "Overlap review", resolveBeforeSaving: "Resolve before saving", idleStatus: "Millisecond precision · host-controlled persistence", discard: "Discard changes", save: "Save timeline", saving: "Saving…" };
const defaultFeatures = { header: true, inheritedIntro: true, mediaPreview: true, zoom: true, addControls: true, inspector: true, validation: true, actions: true };
export function TimelineEditor({ segments, mediaUrl, durationMs, inheritedIntro, allowedTypes = defaultTypes, onChange, onSave, onDiscard, dirty = true, busy = false, statusMessage, readOnly = false, className = "", classNames = {}, labels: suppliedLabels, features: suppliedFeatures, style }) {
    ...
    return (_jsxs("section", { className: `harbor-timeline-editor ${classNames.root ?? ""} ${className}`.trim(), style: style, children: [features.header ? _jsxs("header", { className: classNames.header, children: [_jsxs("div", { children: [_jsx("p", { children: labels.eyebrow }), _jsx("h2", { children: labels.title }), _jsx("span", { children: labels.description })] }), _jsxs("strong", { children: [segments.length, " segment", segments.length === 1 ? "" : "s"] })] }) : null, inheritedIntro && features.inheritedIntro ? _jsxs("div", { className: `harbor-timeline-inherited ${classNames.inherited ?? ""}`.trim(), children: [_jsx("p", { children: labels.inheritedPresentation }), _jsxs("span", { children: [inheritedIntro.title, " \u00B7 ", formatMediaTime(inheritedIntro.durationMs)] }), _jsxs("span", { children: ["Black / silence \u00B7 ", (inheritedIntro.blackGapMs / 1_000).toFixed(1), "s"] }), _jsx("small", { children: labels.inheritedHelp })] }) : null, features.mediaPreview ? mediaUrl ? _jsxs("div", { className: `harbor-timeline-preview ${classNames.preview ?? ""}`.trim(), children: [_jsx("video", { ref: videoRef, src: mediaUrl, preload: "metadata", onLoadedMetadata: (event) => setMeasuredDurationMs(Math.round(event.currentTarget.duration * 1_000)), onTimeUpdate: (event) => { const next = Math.round(event.currentTarget.currentTime * 1_000); setTimeMs(next); if (previewEndMs.current !== null && next >= previewEndMs.current) {
                            ...
                            event.currentTarget.pause();
                                    ...
                                    video.pause(); }, "aria-label": playing ? "Pause preview" : "Play preview", children: playing ? "Ⅱ" : "▶" }), _jsxs("span", { children: [formatMediaTime(timeMs, true), " / ", formatMediaTime(duration, true)] }), _jsx("input", { "aria-label": "Timeline playhead", type: "range", min: 0, max: duration, step: 10, value: Math.min(timeMs, duration), onChange: (event) => seek(Number(event.target.value)) })] })] }) : _jsx("div", { className: `harbor-timeline-empty ${classNames.emptyPreview ?? ""}`.trim(), children: labels.emptyPreview }) : null, _jsxs("div", { className: `harbor-timeline-track-heading ${classNames.trackHeading ?? ""}`.trim(), children: [_jsx("strong", { children: labels.sourceTimeline }), features.zoom ? _jsxs("label", { children: [labels.zoom, " ", _jsx("input", { "aria-label": "Timeline zoom", type: "range", min: 1, max: 8, step: 1, value: zoom, onChange: (event) => setZoom(Number(event.target.value)) }), _jsxs("span", { children: [zoom, "\u00D7"] })] }) : null, _jsx("time", { children: formatMediaTime(duration, true) })] }), _jsx("div", { className: `harbor-timeline-track-scroll ${classNames.trackScroll ?? ""}`.trim(), children: _jsxs("div", { ref: trackRef, className: `harbor-timeline-track ${classNames.track ?? ""}`.trim(), style: { width: `${zoom * 100}%` }, onPointerDown: (event) => seekFromPointer(event.clientX), children: [_jsx("span", { className: "harbor-timeline-playhead", style: { left: `${Math.min(100, timeMs / duration * 100)}%` } }), segments.map((segment) => _jsxs("div", { className: `harbor-timeline-segment harbor-timeline-segment--${segment.segmentType} ${selectedId === segment.id ? "is-selected" : ""}`, style: { left: `${segment.startMs / duration * 100}%`, width: `${Math.max(0.4, (segment.endMs - segment.startMs) / duration * 100)}%` }, children: [_jsx("button", { type: "button", onPointerDown: (event) => event.stopPropagation(), onClick: () => setSelectedId(segment.id), children: segment.internalName || titleCase(segment.segmentType) }), readOnly ? null : _jsxs(_Fragment, { children: [_jsx("button", { type: "button", "aria-label": `Drag start of ${segment.internalName || segment.segmentType}`, className: "harbor-timeline-handle harbor-timeline-handle--start", onPointerDown: (event) => beginResize(event, segment, "start") }), _jsx("button", { type: "button", "aria-label": `Drag end of ${segment.internalName || segment.segmentType}`, className: "harbor-timeline-handle harbor-timeline-handle--end", onPointerDown: (event) => beginResize(event, segment, "end") })] })] }, segment.id))] }) }), readOnly || !features.addControls ? null : _jsx("div", { className: `harbor-timeline-add ${classNames.addControls ?? ""}`.trim(), children: allowedTypes.map((type) => _jsxs("button", { type: "button", onClick: () => add(type), disabled: !mediaUrl, children: ["+ ", titleCase(type)] }, type)) }), selected && features.inspector ? _jsx("div", { className: classNames.inspector, children: _jsx(SegmentInspector, { segment: selected, duration: duration, timeMs: timeMs, allowedTypes: allowedTypes, readOnly: readOnly, canPreview: Boolean(mediaUrl), onPatch: (patch) => replace({ ...selected, ...patch }), onPreview: () => { previewEndMs.current = selected.endMs; seek(selected.startMs); void videoRef.current?.play(); }, onDelete: () => { onChange(segments.filter((segment) => segment.id !== selected.id)); setSelectedId(null); } }) }) : null, features.validation && warnings.length ? _jsxs("div", { className: "harbor-timeline-warning", children: [_jsx("strong", { children: labels.overlapReview }), _jsx("span", { children: warnings.map((issue) => issue.message).join(" ") })] }) : null, features.validation && errors.length ? _jsxs("div", { className: "harbor-timeline-errors", role: "alert", children: [_jsx("strong", { children: labels.resolveBeforeSaving }), errors.map((issue, index) => _jsx("span", { children: issue.message }, `${issue.segmentId}:${issue.field}:${index}`))] }) : null, features.actions ? _jsxs("footer", { className: classNames.footer, children: [_jsx("p", { children: statusMessage || labels.idleStatus }), readOnly ? null : _jsxs("div", { children: [onDiscard ? _jsx("button", { type: "button", onClick: onDiscard, disabled: busy || !dirty, children: labels.discard }) : null, onSave ? _jsx("button", { type: "button", className: "harbor-timeline-save", onClick: () => void onSave(segments), disabled: busy || !dirty || errors.length > 0, children: busy ? labels.saving : labels.save }) : null] })] }) : null] }));
}
