...
import type { TimelineEditorProps, TimelineSegment, TimelineSegmentType } from "./types.js";
...
export function TimelineEditor({ segments, mediaUrl, durationMs, inheritedIntro, allowedTypes = defaultTypes, onChange, onSave, onDiscard, dirty = true, busy = false, statusMessage, readOnly = false, className = "" }: TimelineEditorProps) {
  ...
  return (
    <section className={`harbor-timeline-editor ${className}`.trim()}>
      <header><div><p>Playback composition</p><h2>Source timeline</h2><span>Mark the program itself. Inherited presentation media is offset automatically.</span></div><strong>{segments.length} segment{segments.length === 1 ? "" : "s"}</strong></header>
      {inheritedIntro ? <div className="harbor-timeline-inherited"><p>Inherited presentation</p><span>{inheritedIntro.title} · {formatMediaTime(inheritedIntro.durationMs)}</span><span>Black / silence · {(inheritedIntro.blackGapMs / 1_000).toFixed(1)}s</span><small>Read-only here. The host application owns the active platform ident.</small></div> : null}
      {mediaUrl ? <div className="harbor-timeline-preview"><video ref={videoRef} src={mediaUrl} preload="metadata" onLoadedMetadata={(event) => setMeasuredDurationMs(Math.round(event.currentTarget.duration * 1_000))} onTimeUpdate={(event) => { const next = Math.round(event.currentTarget.currentTime * 1_000); setTimeMs(next); if (previewEndMs.current !== null && next >= previewEndMs.current) { previewEndMs.current = null; event.currentTarget.pause(); } }} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} /><div><button type="button" onClick={() => { const video = videoRef.current; if (!video) return; previewEndMs.current = null; if (video.paused) void video.play(); else video.pause(); }} aria-label={playing ? "Pause preview" : "Play preview"}>{playing ? "Ⅱ" : "▶"}</button><span>{formatMediaTime(timeMs, true)} / {formatMediaTime(duration, true)}</span><input aria-label="Timeline playhead" type="range" min={0} max={duration} step={10} value={Math.min(timeMs, duration)} onChange={(event) => seek(Number(event.target.value))} /></div></div> : <div className="harbor-timeline-empty">Attach a video source to unlock scrubbing and segment preview.</div>}
      <div className="harbor-timeline-track-heading"><strong>Source timeline</strong><label>Zoom <input aria-label="Timeline zoom" type="range" min={1} max={8} step={1} value={zoom} onChange={(event) => setZoom(Number(event.target.value))} /><span>{zoom}×</span></label><time>{formatMediaTime(duration, true)}</time></div>
      <div className="harbor-timeline-track-scroll"><div ref={trackRef} className="harbor-timeline-track" style={{ width: `${zoom * 100}%` }} onPointerDown={(event) => seekFromPointer(event.clientX)}><span className="harbor-timeline-playhead" style={{ left: `${Math.min(100, timeMs / duration * 100)}%` }} />{segments.map((segment) => <div key={segment.id} className={`harbor-timeline-segment harbor-timeline-segment--${segment.segmentType} ${selectedId === segment.id ? "is-selected" : ""}`} style={{ left: `${segment.startMs / duration * 100}%`, width: `${Math.max(0.4, (segment.endMs - segment.startMs) / duration * 100)}%` }}><button type="button" onPointerDown={(event) => event.stopPropagation()} onClick={() => setSelectedId(segment.id)}>{segment.internalName || titleCase(segment.segmentType)}</button>{readOnly ? null : <><button type="button" aria-label={`Drag start of ${segment.internalName || segment.segmentType}`} className="harbor-timeline-handle harbor-timeline-handle--start" onPointerDown={(event) => beginResize(event, segment, "start")} /><button type="button" aria-label={`Drag end of ${segment.internalName || segment.segmentType}`} className="harbor-timeline-handle harbor-timeline-handle--end" onPointerDown={(event) => beginResize(event, segment, "end")} /></>}</div>)}</div></div>
      {readOnly ? null : <div className="harbor-timeline-add">{allowedTypes.map((type) => <button type="button" key={type} onClick={() => add(type)} disabled={!mediaUrl}>+ {titleCase(type)}</button>)}</div>}
      {selected ? <SegmentInspector segment={selected} duration={duration} timeMs={timeMs} allowedTypes={allowedTypes} readOnly={readOnly} canPreview={Boolean(mediaUrl)} onPatch={(patch) => replace({ ...selected, ...patch })} onPreview={() => { previewEndMs.current = selected.endMs; seek(selected.startMs); void videoRef.current?.play(); }} onDelete={() => { onChange(segments.filter((segment) => segment.id !== selected.id)); setSelectedId(null); }} /> : null}
      {warnings.length ? <div className="harbor-timeline-warning"><strong>Overlap review</strong><span>{warnings.map((issue) => issue.message).join(" ")}</span></div> : null}
      {errors.length ? <div className="harbor-timeline-errors" role="alert"><strong>Resolve before saving</strong>{errors.map((issue, index) => <span key={`${issue.segmentId}:${issue.field}:${index}`}>{issue.message}</span>)}</div> : null}
      <footer><p>{statusMessage || "Millisecond precision · host-controlled persistence"}</p>{readOnly ? null : <div>{onDiscard ? <button type="button" onClick={onDiscard} disabled={busy || !dirty}>Discard changes</button> : null}{onSave ? <button type="button" className="harbor-timeline-save" onClick={() => void onSave(segments)} disabled={busy || !dirty || errors.length > 0}>{busy ? "Saving…" : "Save timeline"}</button> : null}</div>}</footer>
    </section>
  );
}
