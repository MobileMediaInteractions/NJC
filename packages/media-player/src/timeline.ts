...
export function activeTimelineSegment(segments: PlayerTimelineSegment[], playbackPositionMs: number) {
...
export function validateTimeline(segments: TimelineSegment[], durationMs?: number | null): TimelineValidationIssue[] {
  const issues: TimelineValidationIssue[] = [];
  for (const segment of segments) {
    if (!Number.isInteger(segment.startMs) || segment.startMs < 0) issues.push({ segmentId: segment.id, field: "startMs", message: "Start must be a positive whole number of milliseconds.", severity: "error" });
    if (!Number.isInteger(segment.endMs) || segment.endMs <= segment.startMs) issues.push({ segmentId: segment.id, field: "endMs", message: "End must be after start.", severity: "error" });
    if (durationMs && segment.endMs > durationMs) issues.push({ segmentId: segment.id, field: "endMs", message: "Segment extends beyond the source duration.", severity: "error" });
    if (segment.segmentType === "custom" && !segment.internalName?.trim()) issues.push({ segmentId: segment.id, field: "internalName", message: "Custom segments require an internal name.", severity: "error" });
    if (segment.skippable && segment.segmentType === "custom" && !segment.viewerLabel?.trim()) issues.push({ segmentId: segment.id, field: "viewerLabel", message: "Skippable custom segments require a viewer label.", severity: "error" });
  }
...
export function formatMediaTime(valueMs: number, includeMilliseconds = false) {
  if (!Number.isFinite(valueMs)) return includeMilliseconds ? "00:00.000" : "0:00";
