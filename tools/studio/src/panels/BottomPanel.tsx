...
export function BottomPanel({ active, diagnostics, output, git, performance, packageInfo, detections, tasks, toolchains, trusted, onActive, onTask, onTrust }: {
  active: PanelName;
  ...
  return (
    <section className="bottom-panel" aria-label="Development tools">
      ...
      <div className="bottom-content">
        ...
        {active === "git" && <div className="git-panel"><div className="git-actions"><button>↻ Refresh</button><button disabled>＋ Stage selected</button><button disabled>✓ Commit</button><span>Destructive Git actions require explicit confirmation.</span></div><pre>{git || "Trust the workspace to inspect Git status."}</pre></div>}
