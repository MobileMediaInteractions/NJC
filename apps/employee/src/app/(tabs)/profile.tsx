...
function themeLabel(value: ThemePreference, systemTheme: "light" | "dark") {
  if (value === "system") {
...
export default function ProfileScreen() {
  ...
  async function enablePush() {
    try {
      ...
      await registerEmployeeNotifications(token);
      setNotice("Employee notifications are enabled on this device.");
    } catch (error) {
      setNotice(
  ...
  const choices = adaptiveThemePreferences(theme.systemTheme);
  return (
    <Screen eyebrow="ACCOUNT & SECURITY" title="Profile">
      <Card>
        <Label>Signed in as</Label>
        <Text style={[styles.name, { color: theme.colors.ink }]}>
          {employee.data?.viewer.name}
        </Text>
        <Text style={{ color: theme.colors.muted }}>
          {employee.data?.viewer.email}
        </Text>
        <Text style={{ color: theme.colors.muted }}>
          Role: {employee.data?.viewer.role}
        </Text>
      </Card>
      <Card>
        <Label>Appearance</Label>
        <View style={styles.choices}>
          {choices.map((value: ThemePreference) => (
            <Pressable
              key={value}
            ...
            >
              <Text style={{ color: theme.colors.ink, fontWeight: "800" }}>
                {themeLabel(value, theme.systemTheme)}
              </Text>
            </Pressable>
          ))}
        </View>
      ...
      {notice ? (
        <Text style={{ color: theme.colors.muted }}>{notice}</Text>
      ) : null}
