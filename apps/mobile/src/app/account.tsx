...
import {
  getAudienceMeasurementEnabled,
  setAudienceMeasurementEnabled,
} from "@/lib/audience";
  ...
  if (!process.env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY)
    return (
      <ScrollView contentContainerStyle={styles.content}>
        <Text style={styles.title}>Account services are ready to connect</Text>
        <Text style={styles.copy}>
          Add EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY to enable shared reader and
          newsroom sign-in. Public news, weather and saved stories work without
          an account.
        </Text>
        <LegalLinks />
      </ScrollView>
...
function ConfiguredAccount() {
    ...
    try {
      if (mode === "verify") {
        ...
        if (signIn.status !== "complete")
          throw new Error(
            "This account requires another verification step. Finish sign-in on the website, then return to the app.",
          );
  ...
  async function alerts() {
    ...
    try {
      ...
      await registerForAlerts(token);
      setNotice("Breaking-news alerts are enabled on this device.");
    } catch (error) {
      setNotice(messageFrom(error));
    } finally {
      setBusy(false);
  ...
  if (isSignedIn)
    return (
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Pressable
          onPress={() => void Linking.openURL(`${apiBaseUrl}/data-requests`)}
        >
          <Text style={styles.link}>Export or delete my account data</Text>
        </Pressable>
  ...
  return (
    ...
    >
      ...
      <Text style={styles.copy}>
        {mode === "sign-up"
          ? "Accounts are for readers 13 and older. By creating one, you accept the Terms and Privacy Policy."
          : "Sign in to manage alerts and access authorized newsroom controls."}
      </Text>
      {mode !== "verify" ? (
        <>
          <TextInput
...
function AudiencePreference() {
  ...
  useEffect(() => {
    let active = true;
    void getAudienceMeasurementEnabled().then((value) => {
  ...
  async function change(value: boolean) {
    setEnabled(value);
    await setAudienceMeasurementEnabled(value);
  }
  return (
    <View style={styles.audiencePreference}>
      <View style={styles.preferenceCopy}>
        <Text style={styles.legalTitle}>Anonymous audience measurement</Text>
        <Text style={styles.preferenceBody}>
          Share this installation’s platform and last-active time. No reading
          history or advertising identifier is sent.
        </Text>
      </View>
      <Switch
  ...
  return (
    <View style={styles.legal}>
      ...
      {[
        ...
        <Pressable
          key={path}
          onPress={() => void Linking.openURL(`${apiBaseUrl}${path}`)}
        >
