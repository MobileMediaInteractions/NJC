...
export default async function PreviewClubPage() {
  ...
  return <><NjcPlusStudioHeading eyebrow="Early access review" title="Preview Club" description="Private unreleased productions, invited viewers, viewing state and feedback in one server-authorized workspace." /><div className="grid gap-4 md:grid-cols-3"><Metric icon={<Eye />} label="Configured previews" value={rows.length} /><Metric icon={<UsersRound />} label="Invitations" value={rows.reduce((sum, row) => sum + Number(row.invitations), 0)} /><Metric icon={<MessageSquareText />} label="Responses" value={rows.reduce((sum, row) => sum + Number(row.feedback), 0)} /></div><Card className="mt-6"><CardHeader><CardTitle>Preview productions</CardTitle><CardDescription>Open a production to manage its source timeline, global-intro inheritance, invitations and survey.</CardDescription></CardHeader><CardContent>{rows.length ? <div className="divide-y">{rows.map((row) => <Link key={row.configuration.id} href={`/studio/njc-plus/content/${row.content.id}`} className="grid gap-2 py-4 hover:text-primary md:grid-cols-[1fr_auto]"><div><p className="font-semibold">{row.content.title}</p><p className="text-xs capitalize text-muted-foreground">{row.content.kind.replaceAll("_", " ")} · {row.content.status} · {row.configuration.enabled ? "Accepting authorized viewers" : "Disabled"}</p></div><p className="text-xs text-muted-foreground">{Number(row.invitations)} invited · {Number(row.feedback)} responded</p></Link>)}</div> : <div className="py-12 text-center"><UsersRound className="mx-auto text-muted-foreground" /><h2 className="mt-3 font-semibold">No Preview Club productions</h2><p className="text-sm text-muted-foreground">Create or open NJC+ content, then enable Preview Club.</p></div>}</CardContent></Card></>;
}
