      ...
      <div className="max-w-6xl">
        ...
        <div className="mt-7 grid gap-6 lg:grid-cols-[minmax(0,1.25fr)_minmax(20rem,0.75fr)]">
          ...
          <div className="space-y-6">
            ...
            <Card><CardHeader><CardTitle>Newsroom record</CardTitle><CardDescription>Postgres cache used by publishing and employee tools.</CardDescription></CardHeader><CardContent>{account.databaseProfile ? <div className="space-y-3"><BooleanRow label="Active staff record" value={account.databaseProfile.isActive} /><BooleanRow label="Public staff profile" value={Boolean(account.databaseProfile.publicProfilePublishedAt)} /><DetailRow label="Public slug" value={account.databaseProfile.publicSlug ?? "Not published"} mono /><DetailRow label="Biography" value={account.databaseProfile.bio ? `${account.databaseProfile.bio.length} characters` : "Not written"} /><DetailRow label="Database ID" value={account.databaseProfile.id} mono /><DetailRow label="Created" value={formatDate(account.databaseProfile.createdAt)} /><DetailRow label="Updated" value={formatDate(account.databaseProfile.updatedAt)} /></div> : <div className="flex gap-3 rounded-lg border border-dashed p-4 text-sm text-muted-foreground"><UserRound className="mt-0.5 size-5 shrink-0" /><p>This reader has no newsroom record. Assigning a staff role creates one automatically.</p></div>}</CardContent></Card>
          </div>
        </div>
      </div>
    </StudioShell>
