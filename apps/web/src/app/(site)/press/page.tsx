...
export default function PressPage() {
  const pressPortalUrl = process.env.PRESS_SUBDOMAIN_ENABLED === "true"
    ? process.env.NEXT_PUBLIC_PRESS_URL ?? "https://press.thejerseycourier.com"
    : "/press-portal";
  return (
    <div>
      <section className="border-b bg-brand-navy text-white">
        <div className="container-news grid gap-10 py-14 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:py-20">
          <div><p className="eyebrow text-brand-yellow">Newsroom resources</p><h1 className="headline-balance mt-4 max-w-3xl text-5xl font-black tracking-[-0.055em] sm:text-6xl">The Courier, ready for your newsroom.</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-white/72">Generate a tailored, publication-ready ZIP for reporting, broadcast, podcasts, events or research. Every package includes your request details, a checksum manifest and clear usage guidance.</p><div className="mt-7 flex flex-wrap items-center gap-3"><Link href={pressPortalUrl} className="inline-flex h-11 items-center bg-brand-yellow px-5 text-sm font-bold text-brand-navy transition hover:bg-white">Open the new Press &amp; Media portal</Link><span className="text-xs text-white/55">Need a custom or policy-reviewed package? Start there.</span></div><div className="mt-6 flex flex-wrap gap-x-6 gap-y-3 text-sm text-white/75"><span className="flex items-center gap-2"><Check className="size-4 text-brand-yellow" /> Immediate legacy download remains available</span><span className="flex items-center gap-2"><Check className="size-4 text-brand-yellow" /> SVG and high-resolution artwork</span><span className="flex items-center gap-2"><Check className="size-4 text-brand-yellow" /> Request-specific manifest</span></div></div>
