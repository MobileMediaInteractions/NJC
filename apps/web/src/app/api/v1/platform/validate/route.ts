...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ error: { code: "invalid_request", message: "Validation request is invalid" } }, { status: 400 });
  try {
    const validation = await validateReceipt(parsed.data.receipt, parsed.data.application, parsed.data.productId);
    return NextResponse.json({ data: validation, meta: { apiVersion: "1" } }, { status: validation.ok ? 200 : 403 });
  } catch (error) {
