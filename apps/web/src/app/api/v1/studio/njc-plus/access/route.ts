...
import {
  njcPlusAccessActionInput,
  type NjcPlusEntitlementActionInput,
} from "@/lib/njc-plus-access-input";
...
export async function POST(request: Request) {
  ...
  const parsed = njcPlusAccessActionInput.safeParse(await request.json().catch(() => null));
  ...
  if ("userClerkId" in value) {
    try {
      await getStudioAccount(value.userClerkId);
    } catch {
      return NextResponse.json({
        error: {
          code: "invalid_account",
          message: "The selected Clerk account no longer exists. Search for the account again.",
        },
      }, { status: 400 });
    }
  }
  ...
  const entitlementValue = value as NjcPlusEntitlementActionInput;
  const now = new Date();
