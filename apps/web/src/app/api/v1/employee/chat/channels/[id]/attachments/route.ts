...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!viewer || !hasDatabase() || !process.env.BLOB_READ_WRITE_TOKEN) return NextResponse.json({ error: { code: "service_not_configured", message: "Secure attachments are not configured" } }, { status: 503 });
