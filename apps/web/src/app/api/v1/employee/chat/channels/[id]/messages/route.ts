...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (mentions.length) await db.insert(employeeNotifications).values(mentions.map((recipientClerkId) => ({ recipientClerkId, kind: "mention", title: `Mention from ${viewer.name}`, body: "You were mentioned in an internal conversation.", destination: `njcourier-employee:///v1/chat/channel/${id}` })));
  if (mentions.length) await sendEmployeePush(mentions, `Mention from ${viewer.name}`, `njcourier-employee:///v1/chat/channel/${id}`);
