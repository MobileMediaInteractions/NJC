...
async function readDatasets(): Promise<Dataset[]> {
  ...
  const data = await Promise.all([
    ...
    db.select().from(schema.apiAuditLogs), db.select().from(schema.pushDevices), db.select().from(schema.audienceInstallations), db.select().from(schema.analyticsDailyViews), db.select().from(schema.analyticsPeriodArchives), db.select().from(schema.siteSettings), db.select().from(schema.dataRequests),
  ...
  const definitions = [
    ...
    ["api_audit_logs", schema.apiAuditLogs], ["push_devices", schema.pushDevices], ["audience_installations", schema.audienceInstallations], ["analytics_daily_views", schema.analyticsDailyViews], ["analytics_period_archives", schema.analyticsPeriodArchives], ["site_settings", schema.siteSettings], ["data_requests", schema.dataRequests],
