...
function sanitize(name: string, rows: Record<string, unknown>[]) {
  ...
  if (name === "web_push_subscriptions") return rows.map((row) => ({
    ...row,
    endpoint: `REMOVED:${row.id ?? "subscription"}`,
    p256dh: "REMOVED",
    auth: "REMOVED",
    isActive: false,
  ...
  if (name === "press_kit_requests") return rows.map((row) => ({ ...row, accessTokenHash: null }));
  if (name === "press_kit_packages") return rows.map((row) => ({ ...row, downloadTokenHash: "REMOVED", status: "revoked", revokedAt: row.revokedAt ?? new Date().toISOString() }));
  return rows;
}
...
async function readDatasets(): Promise<Dataset[]> {
  ...
  const data = await Promise.all([
    db.select().from(schema.users), db.select().from(schema.categories), db.select().from(schema.stories), db.select().from(schema.storyRevisions), db.select().from(schema.storyAuthors), db.select().from(schema.linkInBioEntries), db.select().from(schema.pseudonymModerationEvents), db.select().from(schema.storyApprovals), db.select().from(schema.storyPublicationJobs), db.select().from(schema.siteConfigurationRevisions),
    ...
    db.select().from(schema.alerts), db.select().from(schema.liveEvents), db.select().from(schema.liveEventUpdates), db.select().from(schema.liveEventUpdateRevisions), db.select().from(schema.newsTips), db.select().from(schema.twentyUnderTwentyPrograms), db.select().from(schema.twentyUnderTwentySubmissions), db.select().from(schema.pressKitRequests), db.select().from(schema.pressAssets), db.select().from(schema.pressKitRequestAssets), db.select().from(schema.pressKitMessages), db.select().from(schema.pressKitPackages), db.select().from(schema.pressKitAuditLogs), db.select().from(schema.pressReleases), db.select().from(schema.apiKeys),
    db.select().from(schema.apiAuditLogs), db.select().from(schema.pushDevices), db.select().from(schema.webPushSubscriptions), db.select().from(schema.notificationCampaigns), db.select().from(schema.notificationDeliveries), db.select().from(schema.audienceInstallations), db.select().from(schema.audienceInstallationVersions), db.select().from(schema.audiencePresenceEvents), db.select().from(schema.analyticsEvents), db.select().from(schema.analyticsDailyViews), db.select().from(schema.analyticsPeriodArchives), db.select().from(schema.analyticsArchiveRevisions), db.select().from(schema.siteSettings), db.select().from(schema.legalCenterEntries), db.select().from(schema.dataRequests),
    ...
    db.select().from(schema.premiumTimelineSegments), db.select().from(schema.premiumPlatformIntros), db.select().from(schema.premiumPreviewConfigurations), db.select().from(schema.premiumPreviewInvitations), db.select().from(schema.premiumPreviewQuestions), db.select().from(schema.premiumPreviewResponses),
    db.select().from(schema.premiumTiers), db.select().from(schema.premiumOffers), db.select().from(schema.premiumSubscriptions),
    db.select().from(schema.financialSettings), db.select().from(schema.financialProviderEvents), db.select().from(schema.financialLedgerEntries), db.select().from(schema.financialPeriodCloses),
    db.select().from(schema.premiumEntitlements), db.select().from(schema.premiumBetaTesterGrants),
    db.select().from(schema.accessCreditLedger), db.select().from(schema.accessCreditRedemptionRules), db.select().from(schema.accessCreditRedemptions),
    db.select().from(schema.premiumPlaybackProgress), db.select().from(schema.premiumComments), db.select().from(schema.premiumCommentReports), db.select().from(schema.premiumAuditLogs),
    ...
    db.select().from(schema.distributionFiles), db.select().from(schema.distributionPackages), db.select().from(schema.distributionPackageItems),
    db.select().from(schema.distributionGrants), db.select().from(schema.distributionPlaybackProgress), db.select().from(schema.distributionAuditLogs),
    db.select().from(schema.distributionUserLibrary),
  ...
  const definitions = [
    ["users", schema.users], ["categories", schema.categories], ["stories", schema.stories], ["story_revisions", schema.storyRevisions], ["story_authors", schema.storyAuthors], ["link_in_bio_entries", schema.linkInBioEntries], ["pseudonym_moderation_events", schema.pseudonymModerationEvents], ["story_approvals", schema.storyApprovals], ["story_publication_jobs", schema.storyPublicationJobs], ["site_configuration_revisions", schema.siteConfigurationRevisions],
    ...
    ["alerts", schema.alerts], ["live_events", schema.liveEvents], ["live_event_updates", schema.liveEventUpdates], ["live_event_update_revisions", schema.liveEventUpdateRevisions], ["news_tips", schema.newsTips], ["twenty_under_twenty_programs", schema.twentyUnderTwentyPrograms], ["twenty_under_twenty_submissions", schema.twentyUnderTwentySubmissions], ["press_kit_requests", schema.pressKitRequests], ["press_assets", schema.pressAssets], ["press_kit_request_assets", schema.pressKitRequestAssets], ["press_kit_messages", schema.pressKitMessages], ["press_kit_packages", schema.pressKitPackages], ["press_kit_audit_logs", schema.pressKitAuditLogs], ["press_releases", schema.pressReleases], ["api_keys", schema.apiKeys],
    ["api_audit_logs", schema.apiAuditLogs], ["push_devices", schema.pushDevices], ["web_push_subscriptions", schema.webPushSubscriptions], ["notification_campaigns", schema.notificationCampaigns], ["notification_deliveries", schema.notificationDeliveries], ["audience_installations", schema.audienceInstallations], ["audience_installation_versions", schema.audienceInstallationVersions], ["audience_presence_events", schema.audiencePresenceEvents], ["analytics_events", schema.analyticsEvents], ["analytics_daily_views", schema.analyticsDailyViews], ["analytics_period_archives", schema.analyticsPeriodArchives], ["analytics_archive_revisions", schema.analyticsArchiveRevisions], ["site_settings", schema.siteSettings], ["legal_center_entries", schema.legalCenterEntries], ["data_requests", schema.dataRequests],
    ...
    ["premium_timeline_segments", schema.premiumTimelineSegments], ["premium_platform_intros", schema.premiumPlatformIntros], ["premium_preview_configurations", schema.premiumPreviewConfigurations], ["premium_preview_invitations", schema.premiumPreviewInvitations], ["premium_preview_questions", schema.premiumPreviewQuestions], ["premium_preview_responses", schema.premiumPreviewResponses],
    ["premium_tiers", schema.premiumTiers], ["premium_offers", schema.premiumOffers], ["premium_subscriptions", schema.premiumSubscriptions],
    ["financial_settings", schema.financialSettings], ["financial_provider_events", schema.financialProviderEvents], ["financial_ledger_entries", schema.financialLedgerEntries], ["financial_period_closes", schema.financialPeriodCloses],
    ["premium_entitlements", schema.premiumEntitlements], ["premium_beta_tester_grants", schema.premiumBetaTesterGrants],
    ["access_credit_ledger", schema.accessCreditLedger], ["access_credit_redemption_rules", schema.accessCreditRedemptionRules], ["access_credit_redemptions", schema.accessCreditRedemptions],
    ["premium_playback_progress", schema.premiumPlaybackProgress], ["premium_comments", schema.premiumComments], ["premium_comment_reports", schema.premiumCommentReports], ["premium_audit_logs", schema.premiumAuditLogs],
    ...
    ["distribution_files", schema.distributionFiles], ["distribution_packages", schema.distributionPackages], ["distribution_package_items", schema.distributionPackageItems],
    ["distribution_grants", schema.distributionGrants], ["distribution_playback_progress", schema.distributionPlaybackProgress], ["distribution_audit_logs", schema.distributionAuditLogs],
    ["distribution_user_library", schema.distributionUserLibrary],
...
export async function createPortableBackup(options: { passphrase: string; includeMedia?: boolean; maxBytes?: number }) {
  ...
  entry(archive, "config/environment-keys.txt", ["DATABASE_URL", "NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY", "CLERK_SECRET_KEY", "BLOB_READ_WRITE_TOKEN", "PRIVATE_BLOB_READ_WRITE_TOKEN", "PRIVATE_BLOB_STORE_ID", "NEXT_PUBLIC_PLUS_HOST", "NEXT_PUBLIC_COURIER_CUT_HOST", "NEXT_PUBLIC_DISTRIBUTION_HOST", "NEXT_PUBLIC_DISTRIBUTION_URL", "DISTRIBUTION_ENABLED", "NEXT_PUBLIC_PRESS_HOST", "NEXT_PUBLIC_PRESS_URL", "PRESS_SUBDOMAIN_ENABLED", "PRESS_KIT_TOKEN_PEPPER", "PRESS_CONTACT_EMAIL", "PRESS_EMAIL_FROM", "RESEND_API_KEY", "DOMAIN_CONTROL_ENABLED", "DOMAIN_CONTROL_OPERATOR_CLERK_IDS", "DOMAIN_CONTROL_CHALLENGE_SECRET", "VERCEL_API_TOKEN", "VERCEL_PROJECT_ID", "VERCEL_TEAM_ID", "IONOS_DNS_API_TOKEN", "IONOS_DNS_ZONE_ID", "IONOS_DNS_API_URL", "CLOUDFLARE_ACCOUNT_ID", "CLOUDFLARE_WORKERS_AI_TOKEN", "CLOUDFLARE_AI_TEXT_MODEL", "STRIPE_SECRET_KEY", "STRIPE_WEBHOOK_SECRET", "STRIPE_TAX_ENABLED", "STRIPE_BILLING_PORTAL_CONFIGURATION_ID", "NJC_PLUS_INVITED_BETA_LIMIT", "KV_REST_API_URL", "KV_REST_API_TOKEN", "KV_REST_API_READ_ONLY_TOKEN", "KV_URL", "REDIS_URL", "UPSTASH_REDIS_REST_URL", "UPSTASH_REDIS_REST_TOKEN", "API_KEY_PEPPER", "DEVICE_PAIRING_PEPPER", "CRON_SECRET", "NEXT_PUBLIC_WEB_PUSH_VAPID_KEY", "WEB_PUSH_VAPID_PRIVATE_KEY", "WEB_PUSH_CONTACT", "SITE_DESIGN_OVERRIDE", "SITE_DESIGN_PREVIEW_SECRET", "EXPO_PUBLIC_API_URL", "EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY", "EXPO_PUBLIC_TV_API_URL", "READER_IOS_APP_ID", "READER_ANDROID_PACKAGE", "READER_ANDROID_SHA256_CERT_FINGERPRINTS", "EXPO_PUBLIC_EMPLOYEE_API_URL", "EXPO_PUBLIC_EMPLOYEE_LINK_HOST", "EMPLOYEE_MIN_APP_VERSION", "EMPLOYEE_IOS_INSTALL_URL", "EMPLOYEE_ANDROID_INSTALL_URL", "EMPLOYEE_INTERNAL_INSTALL_URL", "EMPLOYEE_IOS_APP_ID", "EMPLOYEE_ANDROID_PACKAGE", "EMPLOYEE_ANDROID_SHA256_CERT_FINGERPRINTS", "PLATFORM_LICENSE_ISSUER", "PLATFORM_LICENSE_AUDIENCE", "PLATFORM_LICENSE_KEY_ID", "PLATFORM_ED25519_PRIVATE_KEY_PEM", "PLATFORM_ED25519_PUBLIC_KEY_PEM", "PLATFORM_ED25519_PREVIOUS_PUBLIC_KEYS_JSON", "PLATFORM_LICENSE_KEY_PEPPER", "PLATFORM_INSTALLATION_PEPPER"].join("\n"));
  entry(
    archive,
    "config/press-legal-environment-keys.txt",
    [
      "PRESS_LEGAL_APPROVED_POLICY_VERSION",
      "PRESS_LEGAL_APPROVED_LICENSE_VERSION",
      "PRESS_LEGAL_ENTITY_NAME",
      "PRESS_LEGAL_JURISDICTION",
      "PRESS_REQUEST_RETENTION_DAYS",
    ].join("\n"),
  );
