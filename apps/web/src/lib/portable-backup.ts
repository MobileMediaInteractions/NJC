...
async function readDatasets(): Promise<Dataset[]> {
  ...
  const data = await Promise.all([
    ...
    db.select().from(schema.premiumTiers), db.select().from(schema.premiumOffers), db.select().from(schema.premiumSubscriptions), db.select().from(schema.premiumEntitlements), db.select().from(schema.premiumBetaTesterGrants),
  ...
  const definitions = [
    ...
    ["premium_tiers", schema.premiumTiers], ["premium_offers", schema.premiumOffers], ["premium_subscriptions", schema.premiumSubscriptions], ["premium_entitlements", schema.premiumEntitlements], ["premium_beta_tester_grants", schema.premiumBetaTesterGrants],
