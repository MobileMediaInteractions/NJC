  ...
  return (
    <div className="dark grid min-h-screen place-items-center bg-[#061f31] p-6 text-foreground">
      <Card className="w-full max-w-md border-white/10 bg-card shadow-2xl">
        ...
        <CardContent>{accessPending ? <Button asChild variant="outline" className="w-full"><Link href="/">Return to the front page</Link></Button> : configured ? <Button asChild className="w-full"><Link href="/studio/sign-in">Sign in to Studio</Link></Button> : <div className="space-y-3 text-xs text-muted-foreground"><p>Required Vercel environment variables:</p><code className="block rounded bg-muted p-3 leading-6">NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY<br />CLERK_SECRET_KEY</code><p>Studio does not provide a demo bypass. Connect Clerk before newsroom access is available.</p></div>}</CardContent>
      </Card>
    </div>
  );
}
