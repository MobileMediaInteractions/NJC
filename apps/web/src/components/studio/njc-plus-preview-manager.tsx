...
export function NjcPlusPreviewManager({ contentId }: { contentId: string }) {
  ...
  async function load() {
    ...
    if (!response.ok) setMessage(payload.error?.message || "Preview Club could not be loaded.");
  ...
  async function action(body: Record<string, unknown>, success: string) {
    ...
    else setMessage(payload.error?.message || "Preview Club could not be updated.");
    setBusy(false);
  }
  function save() { return action({ action: "configure", configuration: { enabled, disclaimer, opensAt: fromLocal(opensAt), expiresAt: fromLocal(expiresAt), questions: questions.map((question, index) => ({ ...question, sortOrder: index })) } }, "Preview Club settings saved and audited."); }
  ...
  return <Card><CardHeader><CardTitle className="flex items-center gap-2"><UsersRound className="size-5" /> Preview Club</CardTitle><CardDescription>Invite existing accounts to unreleased material. Access, expiration and revocation are enforced server-side and remain separate from NJC+ membership.</CardDescription></CardHeader><CardContent className="space-y-6">
    ...
    <div className="flex justify-end"><Button type="button" onClick={() => void save()} disabled={busy || disclaimer.length < 40 || questions.some((question) => question.prompt.length < 3)}>{busy ? <LoaderCircle className="animate-spin" /> : <Save />} Save Preview Club</Button></div>
