      ...
      <div className="grid min-h-screen lg:grid-cols-[16rem_1fr]">
        <aside className="hidden border-r border-white/10 bg-[#061f31] text-white lg:flex lg:flex-col">
              ...
              .map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    className={cn(
                      "flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-white/62 transition-colors hover:bg-white/7 hover:text-white",
                  ...
                  >
                    ...
                    {showTipBadge ? (
                      <span
                        className="ml-auto inline-flex min-w-5 items-center justify-center rounded-full bg-red-600 px-1.5 py-0.5 text-[0.65rem] font-black leading-none text-white"
                        aria-label={`${newTipCount} new news tips`}
                      >
                        {tipBadge}
                      </span>
                    ) : null}
          ...
          <div className="border-t border-white/10 p-4">
            <Link
              href="/"
              className="flex items-center gap-2 text-xs font-semibold text-white/55 hover:text-white"
            >
              <ChevronLeft className="size-3.5" /> View public site
            </Link>
          </div>
        </aside>
              ...
              {tipBadge && viewer.role !== "contributor" ? (
                <Button asChild variant="ghost" size="sm">
                  <Link href="/studio/tips" aria-label={`${newTipCount} new news tips`}>
                    <Inbox />
                    <span className="rounded-full bg-red-600 px-1.5 py-0.5 text-[0.65rem] font-black leading-none text-white">
                      {tipBadge}
                    </span>
                  </Link>
                </Button>
