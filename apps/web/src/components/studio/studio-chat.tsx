...
export function StudioChat({ viewer, initialChannelId }: { viewer: Viewer; initialChannelId: string | null }) {
  ...
  return (
    <div className="grid h-[calc(100dvh-6.25rem)] min-h-[38rem] overflow-hidden rounded-[1.35rem] border border-black/10 bg-card shadow-[0_24px_80px_rgba(3,29,21,.13)] dark:border-white/10 lg:grid-cols-[18rem_minmax(0,1fr)_18rem]">
      <aside className="border-b bg-[linear-gradient(160deg,#061914_0%,#0b2c22_100%)] text-white lg:border-b-0 lg:border-r lg:border-white/10">
        <div className="flex h-[4.5rem] items-center justify-between border-b border-white/10 px-4"><div><p className="text-[0.58rem] font-black uppercase tracking-[0.2em] text-[#d5a341]">Teamspace</p><p className="mt-1 text-sm font-bold">Courier newsroom</p></div>{canManage ? <Dialog open={channelDialogOpen} onOpenChange={setChannelDialogOpen}><DialogTrigger asChild><Button size="icon-sm" variant="ghost" className="text-white/60 hover:bg-white/10 hover:text-white" aria-label="Create channel"><Plus /></Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Create a public channel</DialogTitle><DialogDescription>Everyone with newsroom chat access can find and join this conversation.</DialogDescription></DialogHeader><Input value={channelName} onChange={(event) => setChannelName(event.target.value)} placeholder="Channel name" maxLength={80} onKeyDown={(event) => { if (event.key === "Enter") void createChannel(); }} /><DialogFooter><Button onClick={() => void createChannel()} disabled={!channelName.trim()}>Create channel</Button></DialogFooter></DialogContent></Dialog> : null}</div>
        <div className="max-h-52 overflow-y-auto p-2 lg:max-h-[calc(100vh-8rem)]">
          ...
          {loading ? <div className="flex items-center gap-2 px-2 py-3 text-xs text-white/50"><Loader2 className="size-3.5 animate-spin" /> Loading conversations</div> : channels.length ? channels.map((channel) => <div key={channel.id} className={`group flex w-full items-center rounded-xl border text-sm transition ${channel.id === selectedId ? "border-[#d5a341]/25 bg-[#d5a341]/14 text-[#f6d58f] shadow-[inset_3px_0_0_#d5a341]" : "border-transparent text-white/52 hover:border-white/8 hover:bg-white/7 hover:text-white"}`}><button type="button" onClick={() => void selectChannel(channel.id)} disabled={uploading} aria-pressed={channel.id === selectedId} className="flex min-w-0 flex-1 items-center gap-2 px-3 py-2.5 text-left disabled:cursor-wait"><span className="shrink-0">{channel.kind === "public" ? <Hash className="size-4" /> : <MessageCircle className="size-4" />}</span><span className="min-w-0 flex-1 truncate">{channel.name}</span>{channel.unread ? <span className="inline-flex min-w-5 items-center justify-center rounded-full bg-red-600 px-1.5 py-0.5 text-[0.65rem] font-black text-white">{channel.unread > 9 ? "9+" : channel.unread}</span> : null}</button>{canManage && (channel.kind === "public" || channel.kind === "private") ? <button type="button" onClick={() => beginDeleteChannel(channel)} disabled={uploading} className={`mr-1 inline-flex size-7 shrink-0 items-center justify-center rounded-lg text-red-300 transition hover:bg-red-500/20 hover:text-red-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-300 disabled:cursor-wait ${channel.id === selectedId ? "opacity-100" : "opacity-50 group-hover:opacity-100 group-focus-within:opacity-100"}`} aria-label={`Delete ${channel.name}`} title={`Delete ${channel.name}`}><Trash2 className="size-3.5" /></button> : null}</div>) : <p className="px-2 py-4 text-xs leading-5 text-white/45">No conversations yet. A channel manager can create the first one.</p>}
        </div>
      </aside>

      <section className="flex min-h-[34rem] min-w-0 flex-col bg-[#fbfcf9] dark:bg-[#0d1511]">
        <header className="flex min-h-[4.5rem] flex-wrap items-center justify-between gap-3 border-b bg-card/70 px-4 py-3 backdrop-blur sm:px-5">
          <div className="min-w-0"><h1 className="flex items-center gap-2 truncate text-base font-bold">{selected?.kind === "public" ? <Hash className="size-4 text-primary" /> : <MessageCircle className="size-4 text-primary" />}{selected?.name ?? "Teamspace"}</h1><p className="truncate text-xs text-muted-foreground">{selected?.topic ?? (selected ? "Private, authorized newsroom conversation" : "Choose a conversation to begin")}</p></div>
      ...
      <aside className="hidden border-l bg-[#f4f5f0] dark:bg-[#111b16] lg:block">
        <div className="flex h-[4.5rem] items-center gap-2 border-b px-4"><UsersRound className="size-4 text-primary" /><div><h2 className="text-sm font-bold">Newsroom activity</h2><p className="text-[0.62rem] text-muted-foreground">Presence across every platform</p></div></div>
